  ...
  if (
    ...
    if (dur) {
      ...
      videoData.totalDurationSec = parseInt(
        String(timeToSec(dur) * (videoData.rangeStep / videoData.totalBytes))
      )
      videoData.totalDurationString = secToTime(videoData.totalDurationSec)
      self.postMessage({
...
self.onmessage = async (event: any) => {
  ...
  if (message.messageType) {
    if (message.messageType === 'input') {
      ...
      videoData.dataList = message.data.dataList
