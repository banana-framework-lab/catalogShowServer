const workerFunction = async () => {
  ...
  const axios = new axiosMod.Axios({
  ...
  async function loadVideo(start: number, end: number) {
    axios
      .get(videoData.realUrl, {
        url: videoData.realUrl,
        responseType: 'blob',
        headers: {
      ...
      .then(async (res: any) => {
        videoData.dataList.push(new Uint8Array(await res.data.arrayBuffer()))
      })
  }

  async function transcode() {
    for (const rangeInfo of videoData.rangeLoadInfoList) {
  ...
  self.onmessage = async (event: any) => {
    ...
    if (message.messageType) {
      if (message.messageType === 'input') {
        corePath = message.data.corePath
        workerPath = message.data.workerPath
        wasmPath = message.data.wasmPath
        ffmpeg = ffmpegMod.createFFmpeg({
          log: true,
          mainName: 'main',
          corePath: corePath,
      ...
      } else if (message.messageType === 'pushQueue') {
